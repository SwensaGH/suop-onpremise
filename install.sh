...
# ============================================================
# Generate SIL (Serveiz Integration Layer) config
# ============================================================
generate_sil_config() {
    local config_dir="/opt/suop/config"
    mkdir -p "$config_dir"

    local sil_key
    sil_key=$(openssl rand -hex 32)

    cat > "$config_dir/sil-config.yaml" << SILEOF
# Generated by install.sh $(date -u +%Y-%m-%dT%H:%M:%SZ)
# Serveiz Integration Layer — Full-Stack Deployment
# Full-stack: no external systems, no field mapping needed.
sil:
  boundary_a:
    inbound:
      source_system: "SERVEIZ_MDM"
      transport: REST
      auth:
        type: internal
        header: "X-SIL-Key"
    ...
    outbound:
      enabled: false
      transport: KAFKA
      topic: "sil.meter-reads.outbound"

  boundary_b:
    source_system: "SERVEIZ_UBS"
    transport: REST
    base_url: "http://localhost:8080/api/portal/v1"
    auth:
      type: internal
SILEOF

    echo "SIL_INBOUND_KEY=${sil_key}" >> /opt/suop/.credentials
    echo "  ✓ SIL config generated at $config_dir/sil-config.yaml"
}
...
# ============================================================
# Generate SIL configuration (full-stack deployment)
# ============================================================
generate_sil_config
